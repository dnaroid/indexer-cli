	...
	describe.sequential("document search and task audit", () => {
		...
		it("indexes Markdown anywhere and audits explicitly linked task changes", () => {
			...
			try {
				...
				writeFileSync(configPath, JSON.stringify(offlineConfig));
				const unified = runCLI(["search", "frobnicate", "--mode", "lexical", "--min-score", "0", "--max-files", "8"], { cwd: root });
				expect(unified.exitCode).toBe(0);
				expect(unified.stdout).toContain("domain=code");
				expect(unified.stdout).toContain("domain=document");
				const search = runCLI(["search", "frobnicate", "--domain", "document", "--mode", "lexical", "--min-score", "0"], { cwd: root });
				expect(search.exitCode).toBe(0);
				expect(search.stdout).toContain("notes/feature.md");
				expect(search.stdout).toContain("notes/draft.md");
				expect(search.stdout).not.toContain("domain=code");
				const context = runCLI(["context", "frobnicate", "--mode", "lexical"], { cwd: root });
				expect(context.exitCode).toBe(0);
				expect(context.stdout).toContain("notes/feature.md");
				const audit = runCLI(["audit", "src/feature.ts", "--no-semantic", "--json"], { cwd: root });
				expect(audit.exitCode).toBe(0);
				expect(audit.stdout).toContain("notes/feature.md");
				expect(audit.stdout).toContain("src/feature.ts");
				expect(JSON.parse(audit.stdout).matches).toContainEqual(expect.objectContaining({ path: "notes/feature.md", group: "explicit-active-spec" }));
