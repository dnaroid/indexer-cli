...
function runCLIWithStreams(args: string[]): {
	exitCode: number;
	stdout: string;
	stderr: string;
} {
	const distEntry = path.join(process.cwd(), "dist", "cli", "entry.js");
	const entry = fileExists(distEntry)
		? distEntry
		: path.join(process.cwd(), "src", "cli", "entry.ts");
	const command = entry.endsWith(".ts")
		? { bin: "npx", commandArgs: ["tsx", entry, ...args] }
		: { bin: "node", commandArgs: [entry, ...args] };
	const result = spawnSync(command.bin, command.commandArgs, {
		cwd: TEMP_DIR,
		encoding: "utf-8",
...
describe.sequential("CLI e2e", () => {
	...
	describe("context", () => {
		...
		it("respects --max-deps", () => {
			const result = runCLIWithStreams(["context", "--max-deps", "1"]);

			expect(result.exitCode).toBe(0);
			expect(result.stderr).toContain(
				"Showing 1 of 10 dependencies. Use --max-deps to see more.",
			);
		});
