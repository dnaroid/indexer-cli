...
import type { LanguageCodeChunk, LanguagePlugin, LanguageSymbol } from '../languages/plugin.js';
...
export type BuiltinLanguagePluginId = 'typescript' | 'python' | 'csharp' | 'gdscript';
...
const BUILTIN_LANGUAGE_PLUGIN_FACTORIES: Record<BuiltinLanguagePluginId, () => LanguagePlugin> = {
...
function normalizeImportKind(kind: string): 'import' | 'require' | 'dynamic_import' {
  if (kind === 'require') return 'require';
  if (kind === 'dynamic_import') return 'dynamic_import';
  return 'import';
}

export const createDefaultLanguagePlugins = (pluginIds?: readonly string[]): LanguagePlugin[] => {
  const ids = pluginIds && pluginIds.length > 0 ? pluginIds : DEFAULT_LANGUAGE_PLUGIN_IDS;
  const normalizedIds = Array.from(
    new Set(ids.map((value) => value.trim().toLowerCase()).filter((value) => value.length > 0))
  );
...
export class IndexerEngine {
  ...
  constructor(options: IndexerEngineOptions) {
    ...
    const languagePlugins = options.languagePlugins ?? createDefaultLanguagePlugins();
    ...
    this.indexingOptions = {
      codeExtensions:
        options.indexingOptions?.codeExtensions && options.indexingOptions.codeExtensions.length > 0
  ...
  private isOllamaContextLengthError(error: unknown): boolean {
    ...
    return (
      ...
      (normalized.includes('context length') && normalized.includes('status=400'))
    );
  }
  ...
  private async embedWithContextGuard(contents: string[], operation: string): Promise<number[][]> {
    try {
  ...
  private shouldIndexChunkInVectors(chunk: ChunkRecord): boolean {
    return !this.indexingOptions.skipImportChunksInVectors || chunk.chunkType !== 'imports';
  }
    ...
    for (let index = importEnd; index < lines.length; index += 1) {
      ...
      if (isCSharp) {
        ...
        if (typeMatch) {
          definitions.push({
            line: index + 1,
            chunkType: typeMatch[1] === 'interface' || typeMatch[1] === 'enum' ? 'types' : 'impl',
      ...
      if (isGDScript) {
        ...
        if (classMatch) {
          definitions.push({ line: index + 1, chunkType: 'types', primarySymbol: classMatch[1] });
          continue;
        }
        const funcMatch = line.match(gdFuncPattern);
        if (funcMatch) {
          definitions.push({ line: index + 1, chunkType: 'impl', primarySymbol: funcMatch[1] });
        }
        continue;
      }
  ...
  private deriveLanguageChunks(
    ...
    return this.chunker.chunk({ filePath, content, language: languageId }).map((chunk) => ({
      content: chunk.content,
  ...
  async close(): Promise<void> {
    await Promise.all([this.metadata.close(), this.vectors.close(), this.embedder.close()]);
  }
  ...
  async indexProject(options: IndexProjectOptions): Promise<IndexResult> {
    ...
    try {
      ...
      const filesToIndex = [...options.changedFiles.modified, ...options.changedFiles.added].filter(
        (filePath) => this.indexingOptions.codeExtensions.includes(extname(filePath).toLowerCase())
      );

      const totalFiles = (await this.metadata.listFiles(projectId, snapshotId)).length;
    ...
    await Promise.all([
      ...
      (async () => {
        ...
        const embeddings = await this.embedWithContextGuard(
          vectorChunks.map((chunk) => data.chunksContent.get(chunk.chunkId) || ''),
    ...
    for (let index = 0; index < options.filesToIndex.length; index += batchSize) {
      ...
      const preparedData = await Promise.all(
        batch.map(async (filePath) => {
          try {
            const content = await readFile(join(options.repoRoot, filePath), 'utf8');
      ...
      const validData = preparedData.filter(
        (value: PreparedFileData | null): value is PreparedFileData => value !== null
      );
      if (validData.length > 0) {
        ...
        const vectorChunksWithContext = validData.flatMap((data: PreparedFileData) =>
          data.chunkRecords
            .filter((chunk: ChunkRecord) => this.shouldIndexChunkInVectors(chunk))
        ...
        if (vectorChunksWithContext.length > 0) {
          const embeddings = await this.embedWithContextGuard(
            vectorChunksWithContext.map((item: { content: string }) => item.content),
            options.operation
          );

          await this.vectors.upsert(
            vectorChunksWithContext.map(
              (item: { chunk: ChunkRecord; filePath: string; content: string }, index: number) => ({
    ...
    try {
      ...
      this.churnByFile = new Map(
        Object.entries(churn).map(([filePath, count]) => {
          ...
          return [normalized.startsWith('./') ? normalized.slice(2) : normalized, count];
        })
      );
    } catch (error) {
  ...
  private async prepareFileRecords(options: IndexFileOptions): Promise<PreparedFileData> {
    const { snapshotId, filePath, content } = options;
    ...
    if (languagePlugin) {
      ...
      symbolRecords = symbols.map((symbol: LanguageSymbol) => ({
        ...
        range: {
          start: { line: symbol.range.startLine, character: symbol.range.startCol },
      ...
      const imports = languagePlugin.extractImports(parsed as any) as ImportInfo[];
  ...
  private async prepareIncrementalSnapshot(options: {
    ...
    await this.vectors.copyVectors(
      ...
      options.prevSnapshotId,
      options.newSnapshotId,
      [...new Set([...options.diff.modified, ...options.diff.deleted, ...options.diff.added])]
    );
  }
  ...
  private async deleteProjectVectorsWithRetry(projectId: ProjectId): Promise<void> {
    ...
    for (let attempt = 0; attempt < delays.length; attempt += 1) {
      ...
      try {
        ...
        if (!this.isTransientVectorDeleteError(error) || attempt === delays.length - 1) {
          throw error;
        }
      }
    }
