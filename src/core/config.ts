...
export class ConfigManager {
  ...
  private loadFromProcessEnv(): void {
    ...
    if (env.INDEXER_EMBEDDING_PROVIDER) {
      this.config.embeddingProvider = env.INDEXER_EMBEDDING_PROVIDER.toLowerCase();
    }
    ...
    if (env.INDEXER_EMBEDDING_CONTEXT_SIZE) {
      ...
      if (Number.isFinite(size) && size > 0) this.config.embeddingContextSize = size;
    }
    ...
    if (env.INDEXER_OLLAMA_NUM_CTX) {
      ...
      if (Number.isFinite(numCtx) && numCtx > 0) this.config.ollamaNumCtx = numCtx;
    }
    const concurrencyRaw = env.INDEXER_INDEX_CONCURRENCY ?? env.INDEX_CONCURRENCY;
    if (concurrencyRaw) {
