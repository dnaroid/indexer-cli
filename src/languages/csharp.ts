...
export class CSharpPlugin implements LanguagePlugin {
	...
	constructor() {
		...
		const language = CSharpLanguage as unknown as Parameters<
			Parser["setLanguage"]
		>[0];
		...
		this.walk(ast.tree.rootNode, (node) => {
			...
			if (node.type === "class_declaration") {
				...
				const isUnityComponent =
					isUnity &&
					baseTypes.some(
						(base) => base === "MonoBehaviour" || base === "ScriptableObject",
					);
		...
		for (const node of sortedDeclarations) {
			...
			chunks.push({
				...
				metadata: {
					...
					primarySymbol:
						rawSymbol && !CSHARP_KEYWORDS.has(rawSymbol)
							? rawSymbol
							: undefined,
				},
			});
	...
	getEntrypoints(filePaths: string[]): string[] {
		return filePaths.filter(
			(filePath) =>
				filePath.endsWith("GameManager.cs") ||
				filePath.endsWith("Bootstrap.cs"),
		);
	}
