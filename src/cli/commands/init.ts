  ...
  program
    ...
    .action(async (projectPath?: string) => {
      ...
      try {
        ...
        await writeFile(configPath, `${JSON.stringify(config.getAll(), null, 2)}\n`, 'utf8');
        ...
        const sourceType = (await pathExists(path.join(resolvedProjectPath, '.git')))
          ? 'local'
          : 'local-nogit';
        ...
        console.log(chalk.green(`Initialized indexer-cli in ${resolvedProjectPath}`));
