  ...
  program
    ...
    .action(
      async (
        ...
        const searchEngine = new SearchEngine(metadata, vectors, embedder, resolvedProjectPath);

        try {
          await Promise.all([metadata.initialize(), vectors.initialize(), embedder.initialize()]);
          ...
          if (!snapshot) {
            throw new Error('No completed snapshot found. Run `indexer index` first.');
          }
          ...
          const results = await searchEngine.search(project.id, snapshot.id, query, {
