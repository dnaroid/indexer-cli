...
function checkOllama(): CheckResult {
  if (!cmdExists("ollama")) {
    return {
      ...
      status: "failed",
      detail:
        "Install Ollama manually from https://ollama.com/download and re-run `npx -y indexer-cli setup`.",
    }
  }
...
function printSummary(): void {
  ...
  if (!allOk) {
    console.log(
      red(
        "  Some dependencies failed. Resolve the issues above and re-run `npx -y indexer-cli setup`.",
      ),
    )
    process.exitCode = 1
  } else {
    console.log(
      green(
        "  All dependencies ready. Run `npx -y indexer-cli init` in your project to start.",
      ),
    )
  }
}
