...
import { IndexerEngine, createDefaultLanguagePlugins } from '../../engine/indexer.js';
...
export function registerIndexCommand(program: Command): void {
  program
    ...
    .action(async (projectPath?: string, options?: { full?: boolean; dryRun?: boolean }) => {
      ...
      try {
        ...
        const latestSnapshot = await metadata.getLatestCompletedSnapshot(project.id);
        ...
        if (options?.dryRun) {
          ...
          if (options.full || !latestSnapshot) {
            ...
            const diff = changedFiles ?? { added: [], modified: [], deleted: [] };
            console.log(chalk.green('Dry run complete.'));
            ...
            console.log(chalk.gray(`Changed total: ${countChangedFiles(diff)}`));
          }
        ...
        spinner.text = options?.full ? 'Running full reindex...' : 'Running incremental index...';
        ...
        if (engine) {
          ...
          await Promise.allSettled([metadata.close(), vectors.close(), embedder.close()]);
        }
      }
    });
}
