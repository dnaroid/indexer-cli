...
export function registerStructureCommand(program: Command): void {
  program
    ...
    .action(async (projectPath?: string, options?: { pathPrefix?: string; kind?: string }) => {
      ...
      try {
        ...
        for (const symbol of allSymbols) {
          ...
          if (options?.pathPrefix && !symbol.filePath.startsWith(options.pathPrefix)) {
            continue;
          }
        ...
        if (files.length === 0) {
          console.log(chalk.gray('No indexed files found for the requested filters.'));
          return;
        }
