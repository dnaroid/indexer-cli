...
Run indexer-cli commands BEFORE falling back to grep/glob/find. Indexer understands code semantics, not just text
patterns.
...
**Output:**
`{ indexed, snapshot: { id, status, createdAt, gitRef }, stats: { files, symbols, chunks, dependencies }, languages, symbolKinds }`
